 ...
 */
public class char_Konsuke extends CharacterBase {
    ...
    public char_Konsuke(GameView gv, int posX, int posY, float scale, boolean anim_flg){
        ...
        m_CharacterID = menu_Character.CHAR_KONSUKE_ID;

        m_CharResource.addElement( (BitmapDrawable)gv.getResources().getDrawable(R.drawable.char300_konsuke1) );
        // if anim_flg == false , don't load those image files
        if(anim_flg) {
            m_CharResource.addElement((BitmapDrawable) gv.getResources().getDrawable(R.drawable.char300_konsuke2));
        }
