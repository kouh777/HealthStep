 ...
 */
public class char_Ryu extends CharacterBase {
    ...
    public char_Ryu(GameView gv, int posX, int posY, float scale, boolean anim_flg){
        ...
        m_CharacterID = menu_Character.CHAR_RYU_ID;

        m_CharResource.addElement( (BitmapDrawable)gv.getResources().getDrawable(R.drawable.char303_ryu1) );
        // if anim_flg == false , don't load those image files
        if(anim_flg) {
            m_CharResource.addElement((BitmapDrawable) gv.getResources().getDrawable(R.drawable.char303_ryu2));
        }
