 ...
 */
public class char_Minmi extends CharacterBase {
    ...
    public char_Minmi(GameView gv, int posX, int posY, float scale, boolean anim_flg){
        ...
        m_CharacterID = menu_Character.CHAR_MINMI_ID;

        m_CharResource.addElement( (BitmapDrawable)gv.getResources().getDrawable(R.drawable.char302_minmi1) );
        // if anim_flg == false , don't load those image files
        if(anim_flg) {
            m_CharResource.addElement((BitmapDrawable) gv.getResources().getDrawable(R.drawable.char302_minmi2));
        }
